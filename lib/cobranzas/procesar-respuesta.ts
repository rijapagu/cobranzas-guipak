...
export interface EntradaRespuestaCliente {
  telefono: string;
  mensaje: string;
  canal?: string;
}

export interface ResultadoRespuestaCliente {
  procesado: boolean;
  motivo?: string;
  intencion?: string;
  acuerdo_registrado?: boolean;
  disputa_registrada?: boolean;
  respuesta_en_cola?: boolean;
}
 ...
 * CP-08: Log de todo.
 *
 * Si el teléfono no corresponde a ningún cliente conocido, el mensaje se
 * registra como desconocido y NO se procesa — nunca se atribuye a otro cliente.
 */
export async function procesarRespuestaCliente(
  entrada: EntradaRespuestaCliente
): Promise<ResultadoRespuestaCliente> {
  const { telefono, mensaje, canal = 'WHATSAPP' } = entrada;

  const cliente = await buscarClientePorTelefono(telefono);
  if (!cliente) {
    await logAccion(null, 'MENSAJE_NUMERO_DESCONOCIDO', 'sistema', '0', {
      telefono,
      mensaje: mensaje.substring(0, 100),
    });
    return { procesado: false, motivo: 'Número no asociado a cliente' };
  }
...
async function buscarClientePorTelefono(telefono: string): Promise<{ codigo: string; nombre: string } | null> {
  ...
  if (numLimpio.length < 7) return null;
  ...
  // Número desconocido: NUNCA atribuir a otro cliente (antes había un fallback
  // "mock" que devolvía el primer cliente con gestión — corrupción de datos).
  return null;
}
